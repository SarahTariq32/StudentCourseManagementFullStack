...
public class CourseService : ICourseService
{
    ...
    public CourseService(ICourseRepository repository, IAiCourseService aiCourseService)
    {
        _repository = repository;
    ...
    public async Task<(bool Success, string Message)> CreateEnrollmentRequestAsync(int studentId, int courseId, string requestType, string? reason)
    {
        var result = await _repository.CreateEnrollmentRequestAsync(studentId, courseId, requestType, reason);

        // Invalidate summary cache early so new request immediately reflects on next summary fetch
        if (result.Success)
        {
    ...
    public async Task<(bool Success, string Message)> ProcessEnrollmentRequestAsync(int requestId, bool approve)
    {
        var result = await _repository.ProcessEnrollmentRequestAsync(requestId, approve);

        // Invalidate summary cache early so approved/rejected request is removed from summary cache
        if (result.Success)
        {
            _aiCourseService.InvalidatePendingRequestsSummaryCache();
        }
